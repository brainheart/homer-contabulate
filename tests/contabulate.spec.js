...
test.describe('Page Load', () => {
  test('loads and shows the Homer title', async ({ page }) => {
    await page.goto('/');
    await expect(page).toHaveTitle('Homer — Iliad & Odyssey Tabular Explorer');
  });

  test('opens on the Book view with both works reachable', async ({ page }) => {
    await page.goto('/');
    await waitForDataLoaded(page);
    await page.waitForSelector('#results tbody tr', { timeout: 10000 });
    await expect(page.locator('#gran')).toHaveValue('act');
    await expect(page.locator('#results tbody tr')).toHaveCount(48);
...
test.describe('Segments Search', () => {
  ...
  test('count cells drill down and ancestor cells filter', async ({ page }) => {
    await page.goto('/?gran=play');
    await waitForDataLoaded(page);
    await page.waitForSelector('#results tbody tr', { timeout: 10000 });
    // Works → the 24 books of the Iliad
    await page.locator('#results tbody tr').first().locator('td:nth-child(4) button.drill-link').click();
    await expect(page.locator('#gran')).toHaveValue('act');
    await expect(page.locator('#segmentsTotalInfo')).toContainText('(24 total rows)');
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('starts with 01.Il.');
    // Back un-drills
    await page.goBack();
    ...
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('is Epic');
  });
  ...
  test('vocabulary granularities put n-grams in the rows', async ({ page }) => {
    ...
    await page.fill('#q', 'θεὸς');
    await page.locator('#addColumnBtn').click();
    await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('matches θεὸς');
    const words = await page.locator('#results tbody tr td:first-child').allTextContents();
    expect(words.length).toBeGreaterThan(0);
    expect(words.every(w => w.includes('θε'))).toBeTruthy();
  });
...
test('vocabulary scope survives switching the n-gram size', async ({ page }) => {
  await page.goto('/?gran=word&s_ft_location=%5E01%5C.Il%5C.');
  await waitForDataLoaded(page);
  await page.waitForSelector('#results tbody tr', { timeout: 10000 });
  await expect(page.locator('#segmentsActiveFilters .active-filter-chip')).toContainText('starts with 01.Il.');
